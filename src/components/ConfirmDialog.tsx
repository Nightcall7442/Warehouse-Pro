    ...
    <div style={{
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", gap: "12px", marginBottom: "16px" }}>
          ...
          <div style={{ flex: 1 }}>
            <h2 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "16px", fontWeight: 600, color: "var(--color-text-primary, #111827)", margin: 0 }}>{title}</h2>
            <p style={{ fontSize: "13px", color: "var(--color-text-secondary, #6b7280)", margin: "4px 0 0", lineHeight: "1.5" }}>{message}</p>
          </div>
          <button onClick={onCancel} style={{ padding: "4px", background: "none", border: "none", cursor: "pointer", color: "var(--color-text-tertiary, #9ca3af)" }}>
            <X size={18}/>
          </button>
        </div>
        <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end" }}>
          <button onClick={onCancel} style={{ padding: "8px 16px", borderRadius: "8px", fontSize: "13px", fontWeight: 500, fontFamily: "'DM Sans', sans-serif", color: "var(--color-text-secondary, #6b7280)", background: "var(--color-surface-light, #f8f9fb)", border: "none", cursor: "pointer" }}>Отмена</button>
