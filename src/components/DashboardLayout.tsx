...
export const SectionTitle = memo(function SectionTitle({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <div>
      <h2 style={{ fontFamily: "'Manrope', sans-serif", fontSize: "15px", fontWeight: 700, color: "var(--color-text-primary, #2b2a28)", margin: 0, letterSpacing: "-0.01em" }}>{title}</h2>
...
export const PageHeader = memo(function PageHeader({ title, subtitle, actions }: { title: string; subtitle?: string; actions?: React.ReactNode }) {
  return (
    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
      <div>
        <CardDots />
        <h1 style={{ fontFamily: "'Manrope', sans-serif", fontSize: "24px", fontWeight: 700, color: "var(--color-text-primary, #2b2a28)", letterSpacing: "-0.025em", margin: 0 }}>{title}</h1>
