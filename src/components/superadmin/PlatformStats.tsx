...
export function PlatformStats() {
  ...
  const cards = [
    ...
    { label: "Выручка", value: money(stats?.revenue ?? 0) + " сум", icon: TrendingUp, gradient: "linear-gradient(135deg, var(--color-warning), #d97706)" },
  ];
  return (
      ...
      {stats && (
        <Section title="По тарифам" icon={BarChart3}>
          <div style={{ display: "flex", gap: "20px", flexWrap: "wrap", alignItems: "center" }}>
            {(["trial", "pro", "exclusive"] as const).map(plan => (
