  ...
  return (
    <div className="animate-fade-up order-grid">
      ...
      {/* Корзина на мобильном — выдвижная панель снизу.
          ...
          здесь — .animate-fade-up. Пока она стояла с forwards, последний кадр
          оставался применённым навсегда, а любой transform, кроме none, делает
          элемент точкой отсчёта для position: fixed внутри: панель отсчитывала
          бы «низ экрана» от низа списка товаров, то есть уезжала бы на
          несколько экранов вниз. Заливку с анимаций сняли (index.css), но
          портал остаётся: он нужен и сам по себе — всплыть над чужими слоями,
          не завися от того, что окажется у панели в предках завтра. AppModal
          рисуется порталом по той же причине.
