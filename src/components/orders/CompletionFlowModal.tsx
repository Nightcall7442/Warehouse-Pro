...
export function CompletionFlowModal({
  ...
  // Начальное значение — null, а НЕ resetKey.
  //
  // useState(resetKey) запоминал ТЕКУЩЕЕ значение при первой отрисовке.
  // Если окно появляется сразу открытым и сразу с позициями — а на странице
  // «Заказы» разметка окна стоит под условием наличия данных заказа, то есть
  // до их прихода окна в дереве нет вовсе, — то уже на первой отрисовке
  // lastResetKey равен resetKey. Условие ниже не срабатывало, позиции не
  // заполнялись, и в запрос уходил пустой список: сервер отвечал отказом
  // проверки «deliveredItems должен содержать хотя бы одну позицию».
  //
  // Ошибка была полностью молчаливой: товары в окне видны (они рисуются из
  // props, а не из состояния), сумма верная, разметка и типы в порядке. Не
  // совпадало только то, что уходило на сервер. Завершать заказы через это
  // окно перестали все роли разом.
