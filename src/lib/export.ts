...
export function buildPDFHtml(data: ReportData) {
  ...
  for (const p of data.turnover.slice(0, 10)) {
    html += `<tr><td>${escapeHtml(p.productName)}</td><td class="right">${fmt(Number(p.currentStock))}</td><td class="right">${fmt(Number(p.soldQty))}</td><td class="right bold">${escapeHtml(p.turnoverRate)}x</td><td class="right">${p.daysToSell < 999 ? p.daysToSell : "—"}</td></tr>`;
  }
  html += `</tbody></table></div>`;
