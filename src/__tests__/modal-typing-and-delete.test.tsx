...
describe("форма завершения заказа не стирается сама", () => {
  ...
  it("сброс завязан на состав позиций, а не на массив", () => {
    ...
    // Сброс сравнивает состав позиций, а не сам массив. Форма записи с тех пор
    // сменилась с эффекта на условие при отрисовке, поэтому проверяется то,
    // что важно: с чем сравнивают.
    expect(SRC, "сброс больше не опирается на состав позиций").toContain(
      "const resetKey = open ? itemsKey : null",
    );
    expect(SRC, "сброс снова зависит от самого массива позиций").not.toMatch(
      /\[open, items\]|resetKey = open \? items[^K]/,
    );
  });
});
