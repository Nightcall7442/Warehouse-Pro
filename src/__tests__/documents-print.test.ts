...
describe("печатные документы: числа", () => {
  ...
  it("недовезённое видно в документе, а не только в системе", () => {
    /*
      ...
      пересчитывается по отпущенному (applyPartialDelivery). Вызывающая
      сторона печатает qty = отпущено, и арифметика в строке сходится — но из
      документа пропадал сам факт недостачи: магазин видел «7» там, где
      заказывал 10, без единого слова почему. Поля orderedQty / deliveredQty /
      returnReason в типе были, их не печатал ни один документ.
    */
    printUzWaybill(order({
      items: [{
        name: "Печенье", unit: "pcs", qty: 7, price: 12000, total: 84000,
        orderedQty: 10, deliveredQty: 7, returnReason: "повреждена упаковка",
      }],
      subtotal: 84000, total: 84000,
    }));
    const t = text();
    expect(t).toContain("заказано 10");
    expect(t).toContain("отпущено 7");
    expect(t).toContain("повреждена упаковка");
  });

  it("довезённое полностью не помечается ничем", () => {
    // Иначе пометка стояла бы на каждой строке любой накладной и перестала
    // бы что-либо значить.
    printUzWaybill(order({
      items: [{ name: "Печенье", unit: "pcs", qty: 10, price: 12000, total: 120000, orderedQty: 10, deliveredQty: 10 }],
      subtotal: 120000, total: 120000,
    }));
    expect(text()).not.toContain("заказано");
  });
