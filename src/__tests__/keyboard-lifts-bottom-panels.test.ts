...
describe("модальные окна над клавиатурой", () => {
  ...
  it("форма нового магазина живёт в общем окне, а не в своём", () => {
    /*
      Восемь полей, GPS и заметки на 375×812 не помещались, а прокрутки не
      было: до «Сохранить» нельзя было добраться вовсе. Чинилось это здесь
      же, в самой форме, — своим порталом со своей высотой.

      Теперь форма открывается общим AppModal, а прокрутка, поправка на
      клавиатуру и слой проверены выше — один раз для всех окон. Правило
      сместилось: экран не должен заводить собственное окно.
    */
    const shops = fs.readFileSync(path.resolve(process.cwd(), "src/pages/AgentShops.tsx"), "utf8");
    expect(shops).toContain("<AppModal");
    expect(shops, "своё окно вместо общего — прокрутка и клавиатура снова свои").not.toContain("createPortal");
  });
});
