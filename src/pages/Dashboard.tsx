    ...
    >
      <p
        style={{
          fontSize: "10px",
          fontWeight: 700,
          color: "var(--color-text-tertiary, #8b9bb4)",
          marginBottom: "8px",
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            ...
            <span
              style={{
                fontSize: "12px",
                color: "var(--color-text-secondary, #5a6a7f)",
              }}
            >
              {p.name}
            </span>
...
/* ── Pie Chart Tooltip ───────────────────────────────────────────────────── */
const PieTooltip = memo(function PieTooltip({
  active,
  payload,
}: {
  active?: boolean;
  payload?: Array<{
    name: string;
    value: number;
    payload: { color: string; status: string };
  }>;
}) {
  if (!active || !payload?.length) return null;
  const p = payload[0];
...
function CircularKpiCard({
  ...
  return (
    <div
      className="kpi-hero stagger-children"
      style={{
        ...
        >
          ...
          >
            <circle
              ...
              r="26"
              fill="none"
              stroke="var(--color-border, #c8d0dc)"
              strokeWidth="5"
            />
            <circle
              cx="32"
        ...
        <Bar dataKey="value" fill={color} radius={[4, 4, 0, 0]}>
          {barData.map((_, index) => (
            <Cell
              key={`cell-${index}`}
              fill={color}
              fillOpacity={0.3 + (index / barData.length) * 0.7}
            />
          ))}
        </Bar>
      </BarChart>
...
export default function Dashboard() {
  ...
  const { data: kpis, isLoading } = trpc.dashboard.kpis.useQuery() as {
    data: DashboardKpis | undefined;
    isLoading: boolean;
  };
  const { data: trends } = trpc.dashboard.trends.useQuery({ range });
  const { data: statusData } = trpc.dashboard.statusBreakdown.useQuery() as {
    data: StatusEntry[] | undefined;
  };
  const { data: activity } = trpc.dashboard.activity.useQuery() as {
    data: ActivityEntry[] | undefined;
  };
  const { data: alerts } = trpc.notification.smartAlerts.useQuery() as {
    data: SmartAlert[] | undefined;
  };
  const { data: gamification } = trpc.agent.gamification.useQuery() as {
    data: unknown;
  };
  ...
  const revenueTrend = useMemo(
    () => (trends ?? []).slice(-7).map(tr => Number(tr.revenue)),
    [trends]
  );
  const ordersTrend = useMemo(
    () => (trends ?? []).slice(-7).map(tr => tr.orderCount),
    [trends]
  );
  ...
  const revenueDelta = useMemo(
    () =>
      calcDelta(
        revenueTrend,
        prev7.map(tr => Number(tr.revenue))
      ),
    [calcDelta, revenueTrend, prev7]
  );
  const ordersDelta = useMemo(
    () =>
      calcDelta(
        ordersTrend,
        prev7.map(tr => tr.orderCount)
      ),
    [calcDelta, ordersTrend, prev7]
  );
  const statusTotal = useMemo(
    () => statusData?.reduce((s, d) => s + Number(d.count), 0) ?? 1,
    [statusData]
  );
  ...
  const pieData = useMemo(
    () =>
      statusData?.map((s, i) => ({
        ...
        color:
          STATUS_COLOR[s.status ?? ""] ?? CHART_COLORS[i % CHART_COLORS.length],
        status: s.status,
      })) ?? [],
    [statusData, lang]
  );
  ...
  if (isLoading || !kpis)
    return (
      ...
      >
        ...
        >
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="neo-card" style={{ height: "160px" }} />
          ))}
        </div>
        <div
          style={{
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      >
        {[
          ...
          {
            label: t("nav.deliveries", "Доставки"),
            icon: Truck,
            path: "/deliveries",
            color: "#34c473",
          },
          {
            label: t("nav.warehouse", "Склад"),
            icon: Package,
            path: "/warehouse",
            color: "#d4973a",
          },
          {
            label: t("nav.arrivals", "Приходы"),
            icon: RefreshCw,
            path: "/arrivals",
            color: "#7a6db5",
          },
      ...
      >
        ...
        >
          <CardDots />
          <p className="kpi-hero-label">{t("agent.todayRevenue")}</p>
          <p
            className="kpi-hero-value"
          ...
          {revenueDelta !== 0 && (
            ...
            >
              {revenueDelta > 0 ? (
                <TrendingUp size={14} color="var(--color-success, #34c473)" />
              ) : (
                <TrendingDown size="14" color="var(--color-danger, #e85050)" />
              )}
              <span
                style={{
                  fontSize: "12px",
                  ...
                  color:
                    revenueDelta > 0
                      ? "var(--color-success, #34c473)"
                      : "var(--color-danger, #e85050)",
                }}
              >
        ...
        >
          <CardDots />
          <p className="kpi-hero-label">{t("agent.todayOrders")}</p>
          <p
            className="kpi-hero-value"
            style={{ fontSize: "28px", marginTop: "8px" }}
          >
          ...
          {alerts.slice(0, 4).map((alert, i) => {
            const colors: Record<string, { bg: string; icon: string }> = {
              info: {
                ...
                icon: "var(--color-info, #5a8fad)",
              },
              warning: {
                bg: "var(--color-warning-subtle)",
                icon: "var(--color-warning, #d4973a)",
              },
              danger: {
                bg: "var(--color-danger-subtle)",
                icon: "var(--color-danger, #d45050)",
              },
            };
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{ width: "36px", height: "36px", color: c.icon }}
                >
            ...
            >
              <defs>
                <linearGradient id="gRevenue" x1="0" y1="0" x2="0" y2="1">
                  <stop
                    offset="0%"
                    stopColor="var(--color-primary, #5b6d8a)"
                    stopOpacity={0.2}
                  />
                  <stop
                    offset="100%"
                ...
                <linearGradient id="gOrders" x1="0" y1="0" x2="0" y2="1">
                  <stop
                    offset="0%"
                    stopColor="var(--color-success, #34c473)"
                    stopOpacity={0.2}
                  />
                  <stop
                    offset="100%"
              ...
              <XAxis
                dataKey="date"
                tick={{
                  fill: "var(--color-text-tertiary, #8b9bb4)",
                  fontSize: 11,
                  fontFamily: "'DM Sans', sans-serif",
                }}
                axisLine={false}
              ...
              <Area
                ...
                type="monotone"
                dataKey="revenue"
                stroke="var(--color-primary, #5b6d8a)"
                strokeWidth={2.5}
                fill="url(#gRevenue)"
              ...
              <Area
                ...
                type="monotone"
                dataKey="orders"
                stroke="var(--color-success, #34c473)"
                strokeWidth={2.5}
                fill="url(#gOrders)"
                name={t("orders.title")}
                dot={false}
          ...
          {!activity?.length ? (
            ...
            activity.slice(0, 8).map(e => (
              <div
                ...
                onMouseEnter={ev => {
                  ev.currentTarget.style.background = "rgba(75,108,246,.04)";
                }}
                onMouseLeave={ev => {
              ...
              >
                <span
                  style={{
                    ...
                    flexShrink: 0,
                    background:
                      STATUS_COLOR[e.status ?? "new"] ??
                      "var(--color-border, #c8d0dc)",
                    boxShadow: "var(--shadow-xs)",
                  }}
                />
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    #{e.orderNumber} ·{" "}
                    {e.createdAt ? format(new Date(e.createdAt), "HH:mm") : ""}
                  </p>
                </div>
                <span
                  style={{
