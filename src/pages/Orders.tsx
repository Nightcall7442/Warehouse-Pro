...
export default function Orders() {
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    for (const o of result.data.data) {
      ...
      html += `<tr><td>${escapeHtml(o.orderNumber)}</td><td>${dateStr}</td><td>${escapeHtml(o.shopName) ?? "—"}</td><td>${escapeHtml(o.agentName) ?? "—"}</td><td>${escapeHtml(o.status)}</td><td class="right bold">${fmtNum(Number(o.total ?? 0))}</td></tr>`;
    }
