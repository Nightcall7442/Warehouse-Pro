  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center"
      ...
      <div className="w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl p-6 space-y-5 animate-fade-up"
        style={{ background: "var(--color-surface)", boxShadow: "0 -25px 60px -15px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.05) inset" }}>

        {/* Header */}
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold" style={{ color: "var(--color-text-primary)", fontFamily: "'DM Sans', sans-serif" }}>
              {t("Движение товара", "Mahsulot harakati")}
            </h2>
            <p className="text-sm mt-0.5" style={{ color: "var(--color-text-tertiary)" }}>{productName}</p>
          </div>
        ...
        {/* Current stock badge */}
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ background: "var(--color-surface-light)" }}>
          <Scale size={16} style={{ color: "var(--color-text-tertiary)" }} />
        ...
        {/* Type selector */}
        <div>
          <label className="text-[10px] font-semibold tracking-wider uppercase mb-3 block" style={{ color: "var(--color-text-tertiary)", fontFamily: "'DM Sans', sans-serif" }}>
            ...
            {types.map(opt => {
              ...
              return (
                <button key={opt.value} onClick={() => setType(opt.value)}
                  ...
                  style={{
                    background: active ? `${opt.color}15` : "var(--color-surface-light)",
                  ...
                  <p className="text-xs font-semibold" style={{ color: active ? opt.color : "var(--color-text-primary)" }}>
        ...
        <div>
          ...
          <input type="number" step="0.01" min="0" autoFocus
            ...
            style={{ background: "var(--color-surface-light)", color: "var(--color-text-primary)", fontFamily: "'DM Sans', sans-serif", border: "2px solid transparent" }}
        ...
        <div>
          ...
          <input className="w-full px-4 py-3 rounded-xl outline-none transition-all"
            style={{ background: "var(--color-surface-light)", color: "var(--color-text-primary)", border: "2px solid transparent" }}
                  ...
                  <div key={m.id} className="flex items-start gap-3 py-3 px-4 rounded-xl"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-xs" style={{ color: "var(--color-text-tertiary)", fontFamily: "'DM Sans', sans-serif" }}>
                      ...
                      <div className="flex items-center justify-between gap-2 mt-1">
                        ...
                        {m.notes && <span className="text-xs truncate" style={{ color: "var(--color-text-tertiary)" }}>{m.notes}</span>}
                      </div>
                      {m.referenceType && (
...
export default function Warehouse() {
  ...
  const kpis = [
    { label: t("ПОЗИЦИЙ", "POZITSIYALAR"), value: summary?.totalSKUs ?? "—", icon: Boxes, gradient: "linear-gradient(135deg, #6366F1, #8B5CF6)", sub: t("уникальных товаров", "noyob mahsulotlar") },
    { label: t("ВСЕГО КГ", "JAMI KG"), value: Number(summary?.totalWeight ?? 0).toLocaleString("ru-RU", { maximumFractionDigits: 0 }), icon: Scale, gradient: "linear-gradient(135deg, #3B82F6, #06B6D4)", sub: t("общий вес на складе", "ombordagi umumiy") },
    { label: t("СТОИМОСТЬ СКЛАДА", "OMBOR QIYMATI"), value: valLoading ? "—" : fmt(Number(valuation?.totalCostValue ?? 0).toFixed(0)), icon: DollarSign, gradient: "linear-gradient(135deg, #F59E0B, #F97316)", sub: t("себестоимость остатков", "qoldiq tannarx") },
    { label: t("МЕРТВЫЙ СТОК", "O'LIK STOK"), value: deadStockItems?.length ?? "—", icon: Clock, gradient: deadStockItems && deadStockItems.length > 0 ? "linear-gradient(135deg, #8B5CF6, #6366F1)" : "linear-gradient(135deg, #22C55E, #16A34A)", sub: t("товаров без продаж", "sotilmasdan mahsulotlar") },
    { label: t("МАЛО СТОКА", "KAM STOK"), value: lowCount, icon: lowCount > 0 ? AlertCircle : Package, gradient: lowCount > 0 ? "linear-gradient(135deg, #EF4444, #DC2626)" : "linear-gradient(135deg, #22C55E, #16A34A)", sub: lowCount > 0 ? t("товаров ниже порога", "mahsulot chegaradan past") : t("все в норме", "hammasi yaxshi") },
  ];
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: "var(--color-text-primary)", fontFamily: "'DM Sans', sans-serif" }}>
            {t("Склад", "Ombor")}
          </h1>
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          {activeTab === "stock" && (
            <>
              ...
              <button onClick={() => data?.data && exportToExcel(formatStockValuationForExport(data.data), "stock-valuation", "Оценка склада", t("Оценка стоимости склада", "Ombor qiymati"))}
                className="flex items-center gap-2 text-sm py-2.5 px-5 rounded-xl font-medium transition-all"
                style={{ background: "linear-gradient(135deg, #F59E0B, #F97316)", color: "#fff", boxShadow: "0 4px 16px rgba(245,158,11,0.3)" }}>
          ...
          {activeTab === "deadstock" && (
            <button onClick={() => deadStockItems?.length && exportToExcel(formatDeadStockForExport(deadStockItems), "dead-stock", "Мёртвый сток", t("Мёртвый сток — товары без продаж", "O'lik stok — sotilmasdan mahsulotlar"))}
              className="flex items-center gap-2 text-sm py-2.5 px-5 rounded-xl font-medium transition-all"
              style={{ background: "linear-gradient(135deg, #8B5CF6, #6366F1)", color: "#fff", boxShadow: "0 4px 16px rgba(139,92,246,0.3)" }}>
          ...
          return (
            <div key={k.label} className="kpi-hero" style={{ animationDelay: `${i * 0.05}s` }}>
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-semibold tracking-wider uppercase" style={{ color: "var(--color-text-tertiary)", fontFamily: "'DM Sans', sans-serif" }}>
                  {k.label}
                </span>
      ...
      {/* Tabs */}
      <div className="flex gap-1 p-1 rounded-xl" style={{ background: "var(--color-surface-light)" }}>
        {tabs.map(tab => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            ...
            {tab.count > 0 && (
              <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded-full font-semibold"
                style={{ background: activeTab === tab.key ? "var(--color-primary)" : "var(--color-border-subtle)", color: activeTab === tab.key ? "#fff" : "var(--color-text-tertiary)" }}>
                {tab.count}
              </span>
            )}
          </button>
        ...
        <>
          ...
          {lowCount > 0 && (
            <div className="flex items-center gap-3 px-5 py-4 rounded-2xl"
              style={{ background: "rgba(220,38,38,0.06)", boxShadow: "inset 0 0 0 1px rgba(220,38,38,0.15)" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <input className="w-full py-3 pl-10 pr-4 rounded-xl text-sm outline-none transition-all"
              style={{ background: "var(--color-surface-light)", color: "var(--color-text-primary)", border: "2px solid transparent", fontFamily: "'DM Sans', sans-serif" }}
              onFocus={e => { e.currentTarget.style.borderColor = "var(--color-primary)"; e.currentTarget.style.boxShadow = "0 0 0 4px rgba(99,102,241,0.1)"; e.currentTarget.style.background = "var(--color-surface)"; }}
              onBlur={e => { e.currentTarget.style.borderColor = "transparent"; e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.background = "var(--color-surface-light)"; }}
            ...
            <div className="space-y-3">
              {isLoading
                ? Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="h-28 rounded-2xl animate-pulse" style={{ background: "var(--color-surface-light)" }} />
                  ))
              ...
              <table className="w-full" style={{ borderCollapse: "separate", borderSpacing: 0 }}>
                ...
                <tbody>
                    ...
                    : data?.data.map((item: any) => {
                        ...
                        return (
                          <tr key={item.id} style={low ? { background: "rgba(220,38,38,0.03)" } : undefined}>
                            ...
                            <td className="px-5 py-3.5 text-xs" style={{ color: "var(--color-text-tertiary)", fontFamily: "'DM Sans', sans-serif", borderBottom: "1px solid var(--color-border-subtle)" }}>
                              {item.productCode}
                            </td>
                            ...
                            <td className="px-5 py-3.5 text-sm" style={{ color: "var(--color-text-tertiary)", fontFamily: "'DM Sans', sans-serif", borderBottom: "1px solid var(--color-border-subtle)" }}>
                  ...
                  {!isLoading && data?.data.map((item: any) => (
                    <tr key={`hist-${item.id}`}>
                      <td colSpan={8} className="p-0" style={{ borderBottom: "1px solid var(--color-border-subtle)" }}>
      ...
      {activeTab === "deadstock" && (
        <>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {[7, 14, 30, 60, 90].map(d => (
              <button key={d} onClick={() => setDeadStockDays(d)}
                ...
                style={{
                  background: deadStockDays === d ? "var(--color-primary)" : "var(--color-surface-light)",
                  color: deadStockDays === d ? "#fff" : "var(--color-text-tertiary)",
                }}>
                {d} {t("дн", "kun")}
              </button>
            ))}
          ...
          {deadStockLoading ? (
            <div className="space-y-3">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="h-16 rounded-xl animate-pulse" style={{ background: "var(--color-surface-light)" }} />
              ))}
            </div>
                  ...
                  <div key={item.productId} className="rounded-2xl p-4" style={{ background: bgColor, boxShadow: `inset 0 0 0 1px ${borderColor}`, animation: `slideUp ${0.3 + i * 0.05}s ease forwards` }}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-semibold" style={{ color: "var(--color-text-primary)" }}>{item.productName}</span>
                      </div>
                  ...
                  {deadStockItems.map((item: any) => {
                    ...
                    return (
                      <tr key={item.productId} style={{ background: rowBg }}>
                        ...
                        <td className="px-5 py-3.5 text-sm font-bold" style={{ color: "var(--color-text-primary)", fontFamily: "'DM Sans', sans-serif", borderBottom: "1px solid var(--color-border-subtle)" }}>
                          {Number(item.currentStock ?? 0).toFixed(2)} {unitLabel(item.unit ?? undefined, lang)}
                        </td>
                        <td className="px-5 py-3.5 text-sm font-bold" style={{ color: badgeColor, fontFamily: "'DM Sans', sans-serif", borderBottom: "1px solid var(--color-border-subtle)" }}>
                          {fmt(Number(item.value ?? 0).toFixed(0))}
                        </td>
                        <td className="px-5 py-3.5 text-xs" style={{ color: "var(--color-text-tertiary)", fontFamily: "'DM Sans', sans-serif", borderBottom: "1px solid var(--color-border-subtle)" }}>
          ...
          {deadStockItems && deadStockItems.length > 0 && (
            <div className="flex items-center justify-between text-xs px-2" style={{ color: "var(--color-text-tertiary)" }}>
