  ...
  const handleExportPDF = async () => {
    ...
    if (cogsByProduct.data && cogsByProduct.data.length > 0) {
      ...
      for (const p of cogsByProduct.data) {
        ...
        html += `<tr><td>${escapeHtml(p.productName)}</td><td class="right">${Number(p.totalQty).toFixed(0)}</td><td class="right">${fmtNum(rev)}</td><td class="right">${fmtNum(cost)}</td><td class="right bold">${fmtNum(profit)}</td><td class="right">${margin}%</td></tr>`;
      }
      html += `</tbody></table></div>`;
    }
