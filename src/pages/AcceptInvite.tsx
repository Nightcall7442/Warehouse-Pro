  ...
  if (error || !invite) return (
    <div className="min-h-screen bg-canvas flex flex-col items-center justify-center px-4">
      <div className="neo-card w-full max-w-sm p-8 text-center space-y-4">
        <h1 className="font-display text-xl font-bold text-primary">{tr("Ссылка недействительна","Havola yaroqsiz")}</h1>
  ...
  if (done) return (
    <div className="min-h-screen bg-canvas flex flex-col items-center justify-center px-4">
      <div className="neo-card w-full max-w-sm p-8 text-center space-y-4">
        ...
        <h1 className="font-display text-xl font-bold text-primary">{tr("Добро пожаловать!","Xush kelibsiz!")}</h1>
        <p className="text-secondary text-sm">
  ...
  return (
    <div className="min-h-screen bg-canvas flex flex-col items-center justify-center px-4">
      ...
      <div className="neo-card w-full max-w-[400px] p-8 space-y-5">
        ...
        <div>
          <label className="font-label text-secondary text-[10px] tracking-wider block mb-1">EMAIL</label>
          <input className="neo-input w-full opacity-60 cursor-not-allowed" value={invite.email} disabled/>
        </div>

        <div>
          <label className="font-label text-secondary text-[10px] tracking-wider block mb-1">{tr("ВАШЕ ИМЯ","ISMINGIZ")} *</label>
          <input className="neo-input w-full" placeholder={tr("Имя Фамилия","Ism Familiya")}
            value={name} onChange={e => setName(e.target.value)} autoFocus/>
        </div>

        <div>
          <label className="font-label text-secondary text-[10px] tracking-wider block mb-1">{tr("ПРИДУМАЙТЕ ПАРОЛЬ","PAROL O'YLAB TOPING")} *</label>
