          ...
          <div key={label} style={{ display: "flex", alignItems: "center", flex: 1 }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px", flexShrink: 0 }}>
              <div style={{
                ...
                background: done ? "var(--color-primary)" : active ? "color-mix(in srgb, var(--color-primary) 12%, var(--color-surface))" : "var(--color-surface-light)",
                color: done ? "#fff" : active ? "var(--color-primary)" : "var(--color-text-tertiary)",
                boxShadow: done ? "0 4px 12px rgba(99,102,241,0.3)" : active ? "0 0 0 3px color-mix(in srgb, var(--color-primary) 15%, transparent)" : "none",
              }}>
              ...
              <span style={{
                ...
                color: active ? "var(--color-primary)" : done ? "var(--color-primary)" : "var(--color-text-tertiary)",
              }}>
                {label}
              </span>
            </div>
            {i < labels.length - 1 && (
              <div style={{
                ...
                background: step < current ? "var(--color-primary)" : "var(--color-border-subtle)",
              }} />
            )}
          </div>
        );
...
function StepShop({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      {/* Заголовок */}
      <div>
        <h2 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "20px", fontWeight: 700, color: "var(--color-text-primary)", margin: "0 0 4px", letterSpacing: "-0.02em" }}>
      ...
      {cities.length > 0 && (
        <div style={{ display: "flex", gap: "8px", overflowX: "auto", paddingBottom: "4px" }}>
          <button onClick={() => setSelectedCity(null)} style={{
            ...
            background: !selectedCity ? "linear-gradient(135deg, #6366F1, #8B5CF6)" : "var(--color-surface-light)",
            color: !selectedCity ? "#fff" : "var(--color-text-secondary)",
            border: "none", boxShadow: !selectedCity ? "0 4px 12px rgba(99,102,241,0.3)" : "none",
          }}>
            {t("Все", "Barchasi")}
          </button>
          {cities.map(city => (
            <button key={city} onClick={() => setSelectedCity(selectedCity === city ? null : city)} style={{
              flexShrink: 0, padding: "8px 18px", borderRadius: "24px", fontSize: "13px", fontWeight: 600,
              fontFamily: "'DM Sans', sans-serif", cursor: "pointer", transition: "all 0.25s ease",
              background: selectedCity === city ? "linear-gradient(135deg, #6366F1, #8B5CF6)" : "var(--color-surface-light)",
              color: selectedCity === city ? "#fff" : "var(--color-text-secondary)",
              border: "none", boxShadow: selectedCity === city ? "0 4px 12px rgba(99,102,241,0.3)" : "none",
            }}>
              {city}
            </button>
          ))}
      ...
      <div style={{ position: "relative" }}>
        <Search size={16} style={{ position: "absolute", left: "14px", top: "50%", transform: "translateY(-50%)", color: "var(--color-text-tertiary)", pointerEvents: "none" }} />
        <input
          style={{
            width: "100%", padding: "12px 14px 12px 42px", borderRadius: "14px",
            background: "var(--color-surface-light)", border: "2px solid transparent",
            fontSize: "14px", fontFamily: "'DM Sans', sans-serif", color: "var(--color-text-primary)",
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <p style={{ fontSize: "12px", color: "var(--color-text-tertiary)", fontFamily: "'DM Sans', sans-serif", margin: 0 }}>
      ...
      {isLoading ? (
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} style={{ height: "72px", background: "var(--color-surface-light)", borderRadius: "16px" }} className="animate-pulse" />
          ))}
        </div>
      ) : filtered?.length === 0 ? (
        <div style={{ textAlign: "center", padding: "48px 0" }}>
          <div style={{
            ...
            <Store size={28} style={{ color: "var(--color-text-tertiary)", opacity: 0.4 }} />
          </div>
          <p style={{ fontSize: "14px", fontWeight: 500, color: "var(--color-text-secondary)", margin: "0 0 4px" }}>
          ...
          {filtered?.map((shop: any) => (
            ...
            >
              <div style={{
                ...
                background: shopId === shop.id ? "linear-gradient(135deg, #6366F1, #8B5CF6)" : "var(--color-surface-light)",
                boxShadow: shopId === shop.id ? "0 4px 12px rgba(99,102,241,0.25)" : "none",
              }}>
                <Store size={20} style={{ color: shopId === shop.id ? "#fff" : "var(--color-text-secondary)" }} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: "14px", color: "var(--color-text-primary)", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {shop.name}
                </p>
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "12px", color: "var(--color-text-secondary)", margin: "3px 0 0" }}>
                  {shop.ownerName ?? "—"}
                  {shop.district ? ` · ${shop.district}` : ""}
                  {shop.city ? `, ${shop.city}` : ""}
                </p>
                {Number(shop.debt ?? 0) > 0 && (
                  <div style={{ display: "inline-flex", alignItems: "center", gap: "4px", marginTop: "6px", padding: "3px 8px", borderRadius: "6px", background: "rgba(220,38,38,0.08)" }}>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: inCart ? "color-mix(in srgb, var(--color-primary) 5%, var(--color-surface))" : "var(--color-surface)",
                  border: inCart ? "1px solid color-mix(in srgb, var(--color-primary) 30%, transparent)" : "1px solid var(--color-border-subtle)",
                }}
              ...
              >
                <div style={{
                  ...
                  alignItems: "center", justifyContent: "center", flexShrink: 0,
                  background: inCart ? "color-mix(in srgb, var(--color-primary) 15%, transparent)" : "var(--color-surface-light)",
                }}>
                  <Package size={18} style={{ color: inCart ? "var(--color-primary)" : "var(--color-text-tertiary)" }} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: "11px", color: "var(--color-text-secondary)", margin: "2px 0 0" }}>
                    ...
                    {lowStock && <span style={{ color: "var(--color-warning)", marginLeft: "8px" }}>⚠</span>}
                  </p>
                </div>
                {inCart ? (
                  <span style={{
                    background: "var(--color-primary)", color: "#fff", borderRadius: "8px",
        ...
        {validItems.length === 0 ? (
          ...
          <>
                ...
                <div key={item.productId} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    <button onClick={() => updateQuantity(item.productId, -1)} style={{
                      width: "24px", height: "24px", borderRadius: "6px", border: "1px solid var(--color-border)",
                    ...
                    <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "13px", fontWeight: 600, color: "var(--color-text-primary)", minWidth: "20px", textAlign: "center" }}>
                      {item.quantity}
                    </span>
                    <button onClick={() => updateQuantity(item.productId, 1)} style={{
                      width: "24px", height: "24px", borderRadius: "6px", border: "1px solid var(--color-border)",
                  ...
                  <span style={{ fontSize: "12px", fontWeight: 600, color: "var(--color-text-primary)", minWidth: "60px", textAlign: "right" }}>
            ...
            <div style={{ borderTop: "1px solid var(--color-border-subtle)", paddingTop: "12px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", paddingTop: "8px", borderTop: "1px solid var(--color-border-subtle)" }}>
  ...
  return (
    <div className="space-y-4 animate-fade-up">
      ...
      <div className="panel p-4 space-y-3">
        {/* Shop */}
        <div className="flex items-center gap-2.5 pb-3" style={{ borderBottom: "1px solid var(--color-border-subtle)" }}>
