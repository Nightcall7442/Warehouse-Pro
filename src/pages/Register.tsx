...
function PasswordStrength({ password }: { password: string }) {
  ...
  const color = score < 2 ? "var(--color-danger)" : score < 3 ? "var(--color-warning)" : "var(--color-success)";
  ...
  return (
    <div className="mt-2 space-y-1.5">
      <div className="flex items-center justify-between">
        <div className="h-1.5 flex-1 rounded-full overflow-hidden mr-3" style={{ background: "var(--color-surface-light)" }}>
      ...
      <div className="flex gap-3">
        {checks.map(c => (
          <div key={c.label} className="flex items-center gap-1">
            ...
            <span className="text-[10px]" style={{ color: c.pass ? "var(--color-success)" : "var(--color-text-tertiary)" }}>{c.label}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="min-h-screen flex" style={{ background: "var(--color-canvas)" }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-[52%] p-12 relative overflow-hidden"
        ...
        <div className="relative space-y-10">
          <div>
            <p className="text-[11px] font-semibold tracking-[.14em] uppercase mb-3" style={{ color: "var(--color-success)" }}>
            ...
            {STEPS.map((s, i) => (
              <div key={i} className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm mt-0.5" style={{ color: "var(--color-text-secondary)" }}>{s.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <p className="relative text-xs" style={{ color: "var(--color-text-tertiary)" }}>© 2025 Warehouse Pro</p>
      </div>
        ...
        <div className="w-full max-w-[380px]">
          <div className="mb-8">
            ...
            <p className="text-sm" style={{ color: "var(--color-text-secondary)" }}>
              ...
              <Link to="/login" className="font-medium hover:underline" style={{ color: "var(--color-primary)" }}>{tr("Войти","Kirish")}</Link>
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {[
              ...
              <div key={f.key} className="space-y-1.5">
                <label className="block text-xs font-medium" style={{ color: "var(--color-text-secondary)" }}>{f.label}</label>
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-medium" style={{ color: "var(--color-text-secondary)" }}>{tr("Пароль","Parol")}</label>
