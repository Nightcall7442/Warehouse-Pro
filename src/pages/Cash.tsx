                  ...
                  return (
                    <tr key={h.id} className="row-hover" data-testid={`cash-holder-${h.id}`}>
                      ...
                      <td style={{ ...tdStyle, textAlign: "right", whiteSpace: "nowrap" }}>
                        <button className="neo-btn neo-btn-xs tap" disabled={h.onHand <= 0} onClick={() => onHandover(h)} data-testid={`cash-handover-${h.id}`}><HandCoins size={13} /> {t("Принять сдачу", "Topshiriqni qabul")}</button>
    ...
    <div className="space-y-4">
      <div className="neo-card neo-card-static" style={{ borderRadius: "20px", padding: "18px" }}>
        ...
        <div className="grid sm:grid-cols-5 gap-3 items-end">
          ...
          {/* Главная заливка экрана одна — «Закрыть день» в шапке; сохранить правила — обычная кнопка. */}
          <button className="neo-btn h-11" disabled={save.isPending} onClick={() => save.mutate({ limit: Number(curLimit || 0), deadline: curDeadline, bankConfirmDays: Math.min(60, Math.max(1, Number(curBankDays) || 3)), startDay: curStartDay })}>{t("Сохранить", "Saqlash")}</button>
        </div>
