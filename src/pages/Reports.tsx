...
export default function Reports() {
  ...
  const handleExportPDF = () => {
    ...
    if (byShop && byShop.length > 0) {
      ...
      for (const s of byShop) {
        html += `<tr><td>${escapeHtml(s.shopName) ?? "—"}</td><td class="right">${fmtNum(Number(s.revenue ?? 0))}</td><td class="right">${s.orderCount ?? 0}</td></tr>`;
      }
    ...
    if (topProds && topProds.length > 0) {
      ...
      for (const p of topProds) {
        html += `<tr><td>${escapeHtml(p.productName)}</td><td>${escapeHtml(p.productCode) ?? "—"}</td><td class="right">${Number(p.totalQty).toFixed(0)}</td><td class="right bold">${fmtNum(Number(p.totalRevenue))}</td></tr>`;
      }
      html += `</tbody></table></div>`;
    }
    ...
    if (agents && agents.length > 0) {
      ...
      for (const a of agents) {
        html += `<tr><td>${escapeHtml(a.agentName) ?? `Agent #${a.agentId}`}</td><td class="right">${Number(a.visits)}</td><td class="right">${Number(a.orders)}</td><td class="right bold">${fmtNum(Number(a.revenue ?? 0))}</td></tr>`;
      }
      html += `</tbody></table></div>`;
    }
