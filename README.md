...
| Что | Сколько |
|---|---|
| Проверок | 1603 в 143 файлах |
| Покрытие | 47,6% строк (порог в сборке — 46,5%) |
| Миграций | 49, все в журнале |
| Таблиц | 45 |
| Роутеров tRPC | 39 |
...
Отдельная работа разворачивает **все 49 миграций на пустой базе** — чтобы новое окружение поднималось с нуля, а не только то, что уже работает.

При запуске приложение сверяет журнал миграций с тем, что записано в базе, и сообщает о расхождении. Проверка появилась не от любви к порядку: три миграции когда-то не выполнились молча — они начинались со строки с синтаксисом MariaDB, которую MySQL отвергает, — и расхождение всплыло месяцы спустя совсем в другом месте.
