...
async function reportSkippedMigrations(): Promise<void> {
  try {
    ...
    /**
     * Не записаны и записаны не будут — но их действие в базе есть.
     *
     * Все пятеро пострадали от одного: файл начинался со строки с синтаксисом
     * MariaDB (`DROP INDEX IF EXISTS`, `ADD COLUMN IF NOT EXISTS`), MySQL её
     * отвергал, и выполнение обрывалось. Переписать сами файлы нельзя — они
     * часть истории, и на чистой базе порядок должен воспроизводиться как есть.
     * Поэтому действие достигнуто позже другими миграциями, а здесь записано,
     * чем именно.
     *
     * Проверено по НАБОРУ КОЛОНОК на боевой базе 31.08.2026, а не по именам
     * объектов: `drizzle push` даёт свои имена, и сверка по именам однажды уже
     * дала ложную тревогу — доложили о поломке, которой не было.
     *
     *   0018 — четыре индекса под запросы KPI. Три добавлены миграцией 0046.
     *          Четвёртый, agent_locations(tenant_id, agent_id, created_at), НЕ
     *          добавлен намеренно: ни один запрос под него не подходит, там
     *          агент группируется, а не сравнивается. Разбор — в 0046.
     *   0019 — уникальный индекс остатков заменён в 0046; индексы shops и
     *          sales_targets и ключ payments.order_id появились через
     *          `drizzle push` под своими именами; два оставшихся ключа на
     *          tenants добавлены миграцией 0048.
     *   0020 — индекс orders(deleted_at) в базе есть.
     *   0035 — колонки delivery_result и delivery_notes в базе есть; они и
     *          создавались `drizzle push`, миграция лишь фиксировала их.
     *   0038 — целиком перекрыта миграцией 0039.
     *
     * Держать о них вечное сообщение об ошибке значит приучить читателя
     * пролистывать этот лог — а он нужен именно для того, чтобы его читали.
     * Для всего остального проверка остаётся в силе: новая незаписанная
     * миграция по-прежнему будет видна.
     */
    const superseded = new Set([
      "0018_add_kpi_performance_indexes",
      "0019_fix_schema_issues",
      "0020_add_orders_deleted_at_index",
      "0035_add_delivery_result_notes",
      "0038_daily_plans_visited_at",
    ]);
