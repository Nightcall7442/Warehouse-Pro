...
const JOBS: Job[] = [
  ...
  {
    /*
      Фото по чужим ссылкам (из прайсов поставщиков) — копией к себе. 19.09.2026
      у Serena Trade опустели все карточки: их картинки жили на cdn.ynamdar.com,
      и сайт лёг. Пока чужой сайт лежит — не выходит, повторяется каждую ночь.
    */
    name: "photos-mirror",
    daily: { hour: 4, minute: 45 },
    run: async () => (await import("../services/photo-mirror")).mirrorForeignPhotos(),
  },
  {
    /*
  ...
  {
    /*
      Обмен с 1С: организации с включённым обменом, у которых прошёл их
      интервал (5 мин … сутки, из настроек). Сама работа — каждые пять минут:
      интервал у каждой организации свой, а расписание одно.
    */
    name: "onec-sync",
    everyMinutes: 5,
    run: async () => (await import("../services/onec-sync")).oneCSync.runScheduled(),
  },
];
