...
describe("правило 2: отказ называет вещь, а не её номер в базе", () => {
  ...
  */
  const NOT_FOR_PEOPLE = [
    ...
    "api/lib/onec-bridge.ts",
    /*
      Дверь для остатка. Её ошибки — не отказы человеку, а нарушенный договор
      между нашими же функциями: отрицательное количество там, где направление
      задаёт имя вызываемой функции, или NaN в списке. До прилавка они не
      доходят вовсе — обработчик подменяет текст у INTERNAL_SERVER_ERROR на
      «Внутренняя ошибка сервера» (api/boot.ts), — а тому, кто будет
      разбираться, нужен именно номер товара: по нему ищут строку списка.

      Спрашивать имя тут нечем: дверь не ходит за справочниками, она пишет
      остаток одним запросом.
    */
