...
describe("выполнение плана считается одним счётом", () => {
  ...
  it("пересчёт снимка пишет то же самое", async () => {
    /*
      Снимок в колонках больше никем не читается, но остаётся: если он
      обновляется своим счётом, рано или поздно его кто-нибудь прочтёт и
      получит четвёртое число.
    */
    const caller = salesTargetRouter.createCaller(ctx("operator", 1));
    await caller.recalculateActuals({
      periodType: "monthly", periodStart: "2026-09-01", periodEnd: "2026-09-30",
    });

    expect(updated).toEqual([{
      actualAmount: "800000.00",
      actualOrderCount: 16,
      actualVisitPct: "75.00",
    }]);
  });
