...
describe("магазины и заказы в меню супервайзера", () => {
  it("есть в боковом меню; магазины — ещё и в нижней панели", () => {
    for (const path of ["/shops", "/orders"]) {
      expect(navPaths("supervisor"), `${path} нет в боковом меню`).toContain(path);
    }
    // Нижняя панель — вкладки мобилки (владелец, 25.09.2026): Главная, Карта,
    // Планы, Магазины. Заказы — в боковом меню, оно на телефоне за «Меню».
    expect(bottomPaths("supervisor"), "/shops нет в нижней панели").toContain("/shops");
  });
