...
function evalCond(row: Record<string, unknown>, cond: unknown): boolean {
  ...
  if (c.__kind === "eq") { const f = mapCol(c.col); return f in row ? row[f] === c.val || String(row[f]) === String(c.val) : true; }
  if (c.__kind === "ne") { const f = mapCol(c.col); return !(f in row) || (row[f] !== c.val && String(row[f]) !== String(c.val)); }
  return true;
}
