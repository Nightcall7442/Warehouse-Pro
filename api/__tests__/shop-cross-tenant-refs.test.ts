 ...
 * отчёт по должникам (жил в analytics) соединялись с users по одному лишь
 * `shops.agent_id`, без условия по организации. Достаточно было указать своему
 * магазину чужой agentId и открыть карточку, чтобы получить имя и почту
 * пользователя другой компании. Перебором так выгружалась вся база платформы,
...
describe("чтение: соединение не выдаёт чужую строку", () => {
  ...
  /*
    Отчёт по должникам уехал отсюда целиком: он жил второй ручкой и не
    вызывался ниоткуда, а страница отчётов показывает analytics.debtReport.

    Условие по организации в соединении с users там тоже стоит — за ним
    следит соседний набор, разбирающий analytics-router. Держать здесь
    проверку удалённой ручки значило бы оставить зелёный тест на несуществующем
    коде: он не падает, значит его никто не читает.
  */
});
