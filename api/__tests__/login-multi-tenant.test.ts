...
describe("вход при одном адресе в нескольких организациях", () => {
  it("пускает того, чья запись НЕ первая по id — раньше это было невозможно", async () => {
    ...
    // Токен в теле — то, чем живёт мобильное приложение: оно кладёт его в
    // SecureStore и подставляет в заголовок Authorization. Без этого поля
    // телефон показывал экран входа при каждом холодном старте.
    expect(second.body.token).toBe("тестовый-токен");
  });
