...
describe("HTML чека", () => {
  const data: ReceiptData = {
    ...
    shop: "Магазин «Альфа»", seller: "Курьер", method: "Наличные", paid: 900000, total: 1250000, currency: "сум",
  ...
  it("58 мм, по-русски, строки, итого, оплата, долг, QR на ту же ссылку; экранирование", async () => {
    ...
    for (const s of ["ООО «Ромашка» &lt;b&gt;", "ИНН 301111111", "№150", "16.09.2026, 10:04", "Магазин «Альфа»", "Выдал", "Вода 1,5 л &lt;script&gt;", "10 × 12 500", "125 000", "ИТОГО", "1 250 000 сум", "Наличные", "900 000 сум", "Остаток в долг", "350 000 сум", "<svg", "Проверить чек"]) {
  ...
  it("без долга строки «остаток» нет; без машины — строки «Машина» нет", async () => {
    const html = await receiptHtml({ ...data, paid: 1250000 });
