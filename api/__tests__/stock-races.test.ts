/**
 * Гонки, которые были и больше не воспроизводятся.
 *
 ...
 * Любое переплетение, которое всё равно портило остаток, было сломано ВОПРЕКИ
 * идеальной построчной блокировке — потому что решение принималось по данным,
 * прочитанным до её взятия.
 *
 * Обе гонки исправлены (восстановление заказа и проведение возврата), и
 * проверки описывают правильное поведение. Пока они были живы, здесь стоял
 * it.fails: тест, утверждающий наблюдаемые, то есть неверные числа, закрепляет
 * ошибку — тот, кто её починит, уронил бы «проходящий тест» и мог решить, что
 * сломал он. С it.fails всё наоборот: как только гонка исчезала, vitest говорил
 * «тест ожидался падающим, но прошёл» — это и был сигнал снять пометку. Оба
 * раза он и сработал.
 *
...
describe("гонки остатка при одновременных вызовах", () => {
  ...
  // Исправлено: решение и запись перенесены внутрь транзакции под
  // .for("update"), а UPDATE статуса защищён условием eq(status, "approved").
  it("два одновременных проведения возврата приходуют товар один раз", async () => {
