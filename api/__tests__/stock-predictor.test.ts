...
describe("прогноз исчерпания: заказ, вернувшийся из архива", () => {
  it("окно спроса режется по дате заказа, а не по чему-то ещё", () => {
    /*
      ── Почему это важно именно здесь ──────────────────────────────────────────

      Заказ, возвращённый из архива в работу, начинает второй круг под тем же
      номером, и services/order-reopen двигает его created_at на день возврата.
      Ровно поэтому вторая продажа попадает в окно прогноза: спрос считается по
      created_at.

      Стоило бы кому-нибудь завести здесь свою дату — «дату первой отгрузки»,
      «дату доставки», что угодно, — и вторая продажа снова стала бы невидимой:
      прогноз занижал бы потребление и оставлял полку пустой, а объяснить это
      было бы нечем, потому что заказ в системе есть и он доставлен.

      Проверяются обе границы: спрос за окно берётся именно между двумя
      отсечками по orders.createdAt.
    */
    canned.stock = [stockRow()];
    canned.demand = [{ productId: 1, quantity: "300.000" }];
    ...
    return predictStockouts(1, 30).then(() => {
      const conds = flatten(capturedWhere.demand);
      const byCreatedAt = conds.filter(
        c => (c.__kind === "gte" || c.__kind === "lt") && c.col === orders.createdAt,
      );
      expect(
        byCreatedAt.map(c => c.__kind).sort(),
        "окно спроса режется не по дате заказа — вторая продажа станет невидимой",
      ).toEqual(["gte", "lt"]);
    });
  });
});
