 ...
 *    видят уже обновлённые колонки. available выводится из двух других, значит
 *    обязан стоять ПОСЛЕДНИМ и читать новые значения. Поставь его раньше — он
 *    посчитается от старых, и тождество available + reserved = current_stock
 *    разойдётся молча.
 *
 *    Прежде требование было ОБРАТНЫМ: available правили независимо, и он был
 *    обязан успеть прочитать старый резерв. Перестановка двух строк тихо ломала
 *    деньги, а понять это по коду было нельзя — только по комментарию.
 *
 * 2. ОГРАНИЧИТЕЛЬ. Вернуть можно ровно столько, сколько лежало в резерве: он
 *    не даёт reserved уйти ниже нуля. Прибавь к свободному полное количество
 *    при просевшем резерве — и в остатке появятся единицы, которых на складе
 *    нет. Здесь этого больше не может случиться: свободное не прибавляется, а
 *    выводится от нового резерва.
 *
...
describe.skipIf(!hasRealDb)("дверь для остатка: резерв и снятие", () => {
  ...
  it("снять больше, чем в резерве, нельзя — вернётся только резерв", async () => {
    /*
      ...
      нет: available стал бы 13 при current_stock = 10. Ровно эта беда девять
      раз и жила в коде — `reserved = GREATEST(0, reserved - q), available =
      available + q`.
    */
    await reserve(2);
    await release(9);
  ...
  it("порядок присвоений сохранён: возвращается ПОЛНЫЙ резерв, а не остаток от него", async () => {
    /*
      Тест ловит перестановку SET местами. Поставь available перед reserved — и
      он выведется из ещё СТАРОГО резерва: свободное останется 6 при
      current_stock 10 и reserved 0. Тождество сломано, ошибки нет.

      Это единственный способ поймать такую перестановку: стенды с поддельной
      базой считают арифметику на JavaScript, где порядок присвоений не значит
      ничего.
    */
    await reserve(4);
    await release(4);
