...
const TABLES = [
  ...
  // Прайс-листы: цена заказа читает привязку магазина к списку. Оставленный
  // чужим набором список изменил бы цену строки в соседнем наборе.
  "price_list_assignments", "price_list_items", "price_lists",
  ...
  // Условия оплаты (оклад, ставка): читаются зарплатой; чужая строка дала бы
  // чужой оклад.
  "commissions", "commission_product_rates", "salary_payouts",
  // Журнал действий: по нему проверяется след сторно; чужая запись задвоила бы счёт.
  "audit_log",
  ...
  "loading_list_items", "loading_list_orders", "loading_lists",
  // Связи с 1С: по ним отличают повтор после таймаута от второго круга
  // заказа, и чужая связь дала бы отказ на ровном месте.
  "id_mappings",
  // Настройки и журнал обмена с 1С: очередь чужого набора выгрузила бы чужой заказ.
  "onec_config", "onec_journal", "sync_status",
  // Инвентаризация: документ со строками; чужой черновик сдвинул бы номер ИНВ-N.
  "stock_count_items", "stock_counts",
  // Регистрация с сайта заводит настройки и подписку организации: после
  // TRUNCATE tenants номера идут заново, и чужая строка settings того же
  // номера роняет регистрацию уникальным индексом.
 ...
 */
/*
  Каждый файл набора заново поднимает соединение и прогоняет drizzle-kit
  migrate (npx стартует 2–5 с, на загруженном раннере — дольше). Хук
  beforeAll без своего таймаута укладывался в 10 с по умолчанию не всегда:
  «Hook timed out in 10000ms» без единой строки о причине. Поэтому у
  test:db в package.json стоит --hookTimeout=120000 — один флаг на весь набор
  вместо числа в каждом файле.
*/
