 ...
 *
 * Учёта остатка по партиям: списания по FEFO и отчёта «сгорает через неделю».
 * Остаток меняют девятнадцать мест сырым SQL в двенадцати файлах, единой двери
 * нет, а StockService.reserve/release/deduct — три главные операции склада —
 * не вызывает вообще никто. Параллельный учёт по партиям разошёлся бы с
 * остатком за неделю, и получился бы второй источник правды о деньгах.
 *
 * Поэтому сначала данные на входе, дверь для остатка — отдельной работой.
 * Ratchet ниже стережёт, чтобы сырого SQL по остатку не становилось больше.
 */
...
describe("сырого SQL по остатку не становится больше", () => {
  /*
    Ratchet, а не запрет: девятнадцать мест сырым SQL меняют warehouse_stock, и
    переписать их разом на денежном пути — отдельная работа с отдельным риском.
    Но расползаться дальше это не должно, иначе будущая дверь для остатка
    станет ещё дороже, а партии так и не получат учёта.

    Число опускается по мере переноса мест на общую дверь — как потолок ошибок
    типов в scripts/typecheck.mjs. Выросло — проверка падает и заставляет
    объяснить, зачем понадобилось двадцатое.
  */
  const BASELINE = 19;
  ...
  it("три главные операции склада всё ещё никем не зовутся — это и есть причина", () => {
    /*
