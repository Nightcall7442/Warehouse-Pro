...
describe("публичный REST API проверяет организацию, а не только ключ", () => {
  ...
  /*
    Ключ выписывается на любом тарифе — apiKey.create тариф не смотрит, — так
    что организация заводила его и пользовалась API как купленным. Проверяет
    доступ сам REST, по тарифу.

    Раньше здесь стоял trial. Теперь пробный период показывает продукт целиком
    (решение владельца), поэтому сторожить надо покупаемый тариф: Basic API не
    включает, и ключ на нём открывать ничего не должен.
  */
  it("ключ на тарифе Basic не открывает API", async () => {
    mockDb = makeApiDb(LIVE_KEY, { status: "active", plan: "basic" });
    const res = await callApi("/stock");
    expect(res.status).toBe(403);
  });

  it("на пробном периоде API открыт — это показ продукта, а не дыра", async () => {
    /*
      Ключ ограничен своей организацией, а лимиты пробного — двадцать товаров и
      пятьдесят заказов: выкачать через него нечего, кроме собственных данных.
      Зато покупатель видит, что API работает, до того как за него заплатит.
    */
