...
describe("подтверждение получения", () => {
  ...
  it("уведомление доходит и до телефона", () => {
    /*
      ...
      хватает вебу, но не приложению: экрана уведомлений в нём нет и SSE оно
      не слушает. А зарплату получают агенты и курьеры, то есть ровно те, у
      кого веба нет вовсе: без толчка «сотрудник получает уведомление»
      выполнялось бы только на бумаге.
    */
    const at = KPI_ROUTER.indexOf("  recordPayout:");
    const body = KPI_ROUTER.slice(at, KPI_ROUTER.indexOf("\n  /*", at + 10));
    expect(body, "толчок на телефон не отправляется").toContain("sendPushToUser(person.id, {");
    /*
      Заголовок — одной переменной на оба канала. Две копии строки разъедутся
      при первой же правке текста, и человек получит в шторке одно, а в
      списке уведомлений другое.
    */
    expect(body, "заголовок задан двумя копиями").toContain("const title =");
    // Ошибка отправки не должна ронять выдачу: деньги уже отданы и записаны.
    expect(body, "недоступный Expo уронит запись выплаты").toMatch(/sendPushToUser\([\s\S]*?\.catch\(\(\) => \{\}\)/);
  });
