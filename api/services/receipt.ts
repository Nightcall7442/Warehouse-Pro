...
export async function receiptHtml(d: ReceiptData): Promise<string> {
...
export async function receiptPage(db: Db, token: string): Promise<{ status: number; html: string }> {
  ...
  if (!d) return missing;
  return { status: 200, html: await receiptHtml(d) };
}
