...
/**
 * Запас на округление вверх — «сдачу оставьте себе», в тийинах.
 *
 * Применяется к ОСТАТКУ, а не к сумме заказа. Раньше это был множитель 1.2:
 * по остатку 5 000 000 он пропускал миллион сверху как «округление», а по
 * остатку 300 — всего 60. Округление у наличных одно на все заказы — до
 * купюры, поэтому допуск фиксированный: 50 000 тийинов = 500 сум.
 * Сравнение в целых тийинах, чтобы 0.1 + 0.2 не решало судьбу платежа.
 */
export const CASH_ROUNDING_TOLERANCE_TIYIN = 50_000;
...
export function assertFitsRemainder(orderTotal: number, priorPaid: number, amount: number): void {
  ...
  if (tiyin(amount) > tiyin(remaining) + CASH_ROUNDING_TOLERANCE_TIYIN) {
    throw new Error(
