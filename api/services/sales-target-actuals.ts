 ...
 * `actual_visit_pct`. Заполняет их только `recalculateActuals`, а зовёт её
 * ровно никто: ручка есть, кнопки нет ни в вебе, ни в мобильном.
 *
 ...
 *
 * `recalculateActuals` ходит в базу по два раза на каждый план — так можно,
 * когда её жмут руками раз в месяц. У экрана начальника планов на экране
 * два-три десятка, и тридцать пар запросов на каждое открытие — это уже не
 * отчёт, а нагрузка.
 *
