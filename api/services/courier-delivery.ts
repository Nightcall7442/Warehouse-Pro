...
import { paidForOrder, assertFitsRemainder, splitExcess, recordExcess } from "./payment";
...
/*
  ...
  Раньше это лежало прямо в courier-router.ts — 700 строк правил дела внутри
  транспорта. Правила (замок строки заказа под транзакцией, повтор = «уже
  сделано», сверка суммы с остатком, частичный возврат построчно) перенесены
  сюда без изменений; роутер оставил себе zod и права.
*/
...
async function alreadyDeliveredByMe(
  db: Db, tenantId: number, courierId: number, orderId: number,
...
export interface CompleteDeliveryInput {
  ...
  debtDueDate?: string;
  returnReason?: string;
  returnedItems?: { itemId: number; returnedQty: number }[];
  notes?: string;
}
...
export async function markDelivered(db: Db, tenantId: number, courierId: number, input: MarkDeliveredInput) {
  ...
  if (!order) {
    const done = await alreadyDeliveredByMe(db, tenantId, courierId, input.orderId);
    if (done) {
  ...
  */
  ...
  await db.transaction(async (tx) => {
    ...
    for (const item of items) {
      ...
      */
      await shipStock(tx, {
        tenantId: tenantId, warehouseId: whId,
    ...
    if (input.cashAmount && Number(input.cashAmount) > 0) {
      ...
      const priorPaid = await paidForOrder(tx, tenantId, order.id);
      ...
      if (excess > 0) await recordExcess(tx, tenantId, order.shopId, order.orderNumber, excess, courierId);
    }
...
export async function completeDelivery(db: Db, tenantId: number, courierId: number, input: CompleteDeliveryInput) {
  ...
  await db.transaction(async (tx) => {
    ...
    if (input.result === "paid" || input.result === "partial_paid") {
      ...
      */
      ...
      for (const item of items) {
        ...
        if (returnedQty > qty) {
          throw new Error(
            `Возвращено больше, чем в заказе: «${await productLabel(tx, tenantId, Number(item.productId))}» — ${returnedQty} из ${qty}`,
          );
        }
    ...
    */
    ...
    if (paidAmount > 0) {
      ...
      if (split.excess > 0) await recordExcess(tx, tenantId, order.shopId, order.orderNumber, split.excess, courierId);
    }
  ...
  // Notify CEO
  const [ceo] = await db.select({ id: users.id }).from(users)
    .where(and(eq(users.tenantId, tenantId), eq(users.role, "ceo"))).limit(1);
  if (ceo) {
    await NotificationService.create(db, {
      tenantId: tenantId,
      userId: ceo.id,
...
export async function markFailed(db: Db, tenantId: number, courierId: number, input: MarkFailedInput) {
