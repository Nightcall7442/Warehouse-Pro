...
export const ReportsService = {
  async getDashboardSummary(db: Db, tenantId: number) {
    ...
    const [agentCount, visitsToday, ordersMonth, revenueMonth, sub] = await Promise.all([
  ...
  async getVisitChart(db: Db, tenantId: number, days: number = 30) {
    const cacheKey = `reports:visitChart:${tenantId}:${days}`;
    const cached = cache.get(cacheKey);
    if (cached) return cached;
  ...
  async getPlanCompletion(db: Db, tenantId: number) {
    const cacheKey = `reports:planCompletion:${tenantId}`;
    const cached = cache.get(cacheKey);
    if (cached) return cached;
  ...
  async getAgentPerformance(db: Db, tenantId: number, days: number = 30) {
    const cacheKey = `reports:agentPerf:${tenantId}:${days}`;
    const cached = cache.get(cacheKey);
    if (cached) return cached;
